...
Порты для отдельных контейнеров:
- Grafana — 3000
- Prometheus — 9090
- Loki — 3100
- Alertmanager — 9093
...
- Метрики собираются статически с сервера 2 по внутренним IP адресам и портам.
...
- Файл алертов `alertmanager.yml` размещён и монтируется в контейнер Alertmanager.
...
В ходе хакатона реализована инфраструктура мониторинга информационных систем, включающая сбор метрик, централизованный логгинг, визуализацию данных и автоматические оповещения через Telegram. Развёртывание автоматизировано через Docker Compose и занимает несколько минут на каждом сервере.